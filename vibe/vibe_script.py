...
class GridMesh(object):
    ...
    Edge families:
        - 'asym': edges aligned with surface U direction (asymptotic target)
        - 'geod': edges aligned with surface V direction (geodesic target)
    """
...
def build_param_grid(surface, u_count, v_count):
    ...
    for i in range(u_count):
        for j in range(v_count - 1):
            v0 = i * v_count + j
            v1 = i * v_count + (j + 1)
            edge_family[tuple(sorted((v0, v1)))] = "geod"
...
def add_geodesic_forces(grid, surf_data, forces):
    # Geodesic family runs along V direction (neighbors in +/- j).
    mesh = grid.mesh
    nv = grid.v_count
    for i in range(grid.u_count):
        for j in range(1, grid.v_count - 1):
            idx = i * nv + j
            prev_idx = i * nv + (j - 1)
            next_idx = i * nv + (j + 1)
            lap_vec = (
                rg.Vector3d(mesh.Vertices[prev_idx])
                + rg.Vector3d(mesh.Vertices[next_idx])
                - rg.Vector3d(mesh.Vertices[idx]) * 2.0
            )
            lap_vec = project_to_tangent(lap_vec, surf_data.normals[idx])
            if lap_vec.IsTiny(EPS):
                continue
            forces[idx] += lap_vec * W_GEODESIC
...
def optimize_mesh(surface, grid):
    ...
    for it in range(ITERATIONS):
        ...
        add_geodesic_forces(grid, surf_data, forces)
